...
function getipsb(json) {
    ...
    get('https://cors-anywhere.herokuapp.com/ping.ele.me/api/parseIp?ip=' + json.ip, function (res) {
        res = JSON.parse(res);
...
var getLocalDNS = function () {
    ...
    img.onload = function () {
        get('https://cors-anywhere.herokuapp.com/ping.ele.me/api/ldns?d=' + raw, function (ip) {
            if (!ip) return ldnsEl.textContent = '获取失败，请刷新重试';
            ldnsEl.textContent = ip;
            get('https://cors-anywhere.herokuapp.com/ping.ele.me/api/parseIp?ip=' + ip, function (res) {
                res = JSON.parse(res);
